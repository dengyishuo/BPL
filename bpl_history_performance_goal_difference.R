...
process_data = function(data){
  ...
  #for total goal 
  data = data.frame(data[,1:5],H_level=rep(0,380),A_level=rep(0,380),total_goal=(H_minus_lambda+A_minus_lambda),H_minus_lambda,A_minus_lambda,y=as.factor(difference))
  data
} 
...
#try lag = 3 directly
ann1=ann(data[train,c(3,4,6:11)],data[train,"y"],size =1,maxit =20000,try=100)
summary(ann1)
...
table(predict(ann2,data[-train,],type="class"),data[-train,'y']) #always approximately 50%....
